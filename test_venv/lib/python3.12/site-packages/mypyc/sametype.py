...
def is_same_signature(a: FuncSignature, b: FuncSignature) -> bool:
    return (
        ...
        and all(
            is_same_type(t1.type, t2.type) and t1.name == t2.name for t1, t2 in zip(a.args, b.args, strict=False)
        )
    )
...
def is_same_method_signature(a: FuncSignature, b: FuncSignature) -> bool:
    return (
        ...
        and all(
            ...
            for t1, t2 in zip(a.args[1:], b.args[1:], strict=False)
        )
    )
...
class SameTypeVisitor(RTypeVisitor[bool]):
    ...
    def visit_rtuple(self, left: RTuple) -> bool:
        return (
            ...
            and all(is_same_type(t1, t2) for t1, t2 in zip(left.types, self.right.types, strict=False))
        )
