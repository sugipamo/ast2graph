...
class TestMemoryLeak(PsutilTestCase):
    ...
    def _check_mem(self, fun, times, retries, tolerance):
        ...
        for idx in range(1, retries + 1):
            ...
            msg = f"Run #{idx}: extra-mem={bytes2human(mem)}, per-call={bytes2human(mem / times)}, calls={times}"
            messages.append(msg)
...
def print_sysinfo():
    ...
    info['memory'] = f"{int(mem.percent)}%%, used={bytes2human(mem.used)}, total={bytes2human(mem.total)}"
    swap = psutil.swap_memory()
    info['swap'] = f"{int(swap.percent)}%%, used={bytes2human(swap.used)}, total={bytes2human(swap.total)}"
