...
def cpu_percent(interval=None, percpu=False):
    ...
    # per-cpu usage
        ...
        for t1, t2 in zip(tot1, _last_per_cpu_times[tid], strict=False):
...
def cpu_times_percent(interval=None, percpu=False):
    ...
    *interval* and *percpu* arguments have the same meaning as in
    ...
    # per-cpu usage
        ...
        for t1, t2 in zip(tot1, _last_per_cpu_times_2[tid], strict=False):
            ret.append(calculate(t1, t2))
        return ret
...
def disk_io_counters(perdisk=False, nowrap=True):
    ...
    if perdisk:
        ...
        return nt(*(sum(x) for x in zip(*rawdict.values(), strict=False)))
...
def net_io_counters(pernic=False, nowrap=True):
    ...
    if pernic:
        ...
        return _common.snetio(*[sum(x) for x in zip(*rawdict.values(), strict=False)])
