...
from ast2graph.api import parse_code, parse_directory, parse_file, parse_files_stream
from ast2graph.exceptions import ParseError
...
class TestParseDirectory:
    ...
    def test_parse_directory_basic(self, tmp_path):
        ...
        assert any("file1.py" in key for key in result)
        assert any("file2.py" in key for key in result)
        assert any("file3.py" in key for key in result)
    ...
    def test_parse_directory_pattern(self, tmp_path):
        ...
        assert len(result) == 1
        assert any("test_file.py" in key for key in result)
    ...
    def test_parse_directory_with_errors(self, tmp_path):
        ...
        # 正常なファイルは処理される
        assert len(result) >= 1
        assert any("good.py" in key for key in result)
    ...
    def test_parse_directory_parallel(self, tmp_path):
        ...
        for i in range(10):
            assert any(f"file{i}.py" in key for key in result)
