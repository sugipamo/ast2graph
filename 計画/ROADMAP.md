...
- [ ] **テスト失敗の修正** `[優先度: 緊急]` [進行中: 20/193テスト失敗中]
  - [ ] include_dependenciesパラメータの実装
  - [ ] エンコーディング処理の修正（UTF-8 BOM、Latin-1対応）
  - [ ] ストリーミング処理の完全実装
  - [ ] 決定性保証の強化（ノードID生成、処理順序）
...
- ✅ メモリ使用量500MB以下
- ✅ 100%決定的動作
- ✅ テストカバレッジ90%以上（89.6%達成、193テスト中173成功）
...
8. ~~依存関係抽出機能の設計と実装~~ ✅ 完了
9. ~~統合テストの作成~~ ✅ 完了
10. **テスト失敗の修正（20個のテスト失敗）** ← 次の優先タスク
11. バッチ処理・並列処理の実装
12. パフォーマンス最適化とメモリ効率化
13. APIドキュメントの作成
14. CLIツールの実装
